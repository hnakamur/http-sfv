...
{
    return hsfv_bare_item_eq(&self->bare_item, &other->bare_item) && hsfv_parameters_eq(&self->parameters, &other->parameters);
}
                           ...
                           const char **out_rest)
{
    ...
    err = hsfv_parse_bare_item(&item->bare_item, allocator, input, input_end, &input);
    if (err) {
        return err;
    }

    err = hsfv_parse_parameters(&item->parameters, allocator, input, input_end, &input);
    if (err) {
        goto error;
    }
