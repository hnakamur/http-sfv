...
hsfv_err_t hsfv_parse_field_value(hsfv_field_value_t *field_value,
  ...
  switch (field_type) {
  case HSFV_FIELD_VALUE_TYPE_LIST:
    err = hsfv_parse_list(&field_value->list, allocator, input, input_end,
                          &input);
    if (err) {
      return err;
    }
    ...
    break;
  case HSFV_FIELD_VALUE_TYPE_DICTIONARY:
    err = hsfv_parse_dictionary(&field_value->dictionary, allocator, input,
                                input_end, &input);
    if (err) {
      return err;
    }
    ...
    break;
  case HSFV_FIELD_VALUE_TYPE_ITEM:
    err = hsfv_parse_item(&field_value->item, allocator, input, input_end,
                          &input);
    if (err) {
      return err;
    }
