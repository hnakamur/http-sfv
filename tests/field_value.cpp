...
#define OK_HELPER(section, input_literal, want)                                \
  ...
  OK_HELPER(
      ...
      (hsfv_field_value_t{
          ...
          .dictionary =
              hsfv_dictionary_t{.members = NULL, .len = 0, .capacity = 8}}),
      true);
  OK_HELPER("non empty dict",
            (hsfv_field_value_t{.type = HSFV_FIELD_VALUE_TYPE_DICTIONARY,
                                .dictionary =
                                    hsfv_dictionary_t{.members = &dict_member,
                                                      .len = 1,
                                                      .capacity = 8}}),
            false);
  OK_HELPER("empty list",
            (hsfv_field_value_t{
                ...
                .list = hsfv_list_t{.members = NULL, .len = 0, .capacity = 8}}),
            true);
  OK_HELPER("non empty list",
            (hsfv_field_value_t{.type = HSFV_FIELD_VALUE_TYPE_LIST,
                                .list = hsfv_list_t{.members = &list_member,
                                                    .len = 1,
                                                    .capacity = 8}}),
            false);
  OK_HELPER("item",
...
#define OK_HELPER(section, input_literal, want)                                \
  SECTION("section") {                                                         \
    ...
    hsfv_err_t err;                                                            \
    err = hsfv_serialize_field_value(&field_value, &hsfv_global_allocator,     \
                                     &buf);                                    \
    CHECK(err == HSFV_OK);                                                     \
